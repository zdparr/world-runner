...
export const BRINECROSS_TEMPLATE: CampaignTemplate = {
  id: 'brinecross',
  name: 'Brinecross',
  description:
...
/** Creates the demo campaign (with Kael). Returns its id, or null if it exists and `reset` is false. */
export async function seedDemoCampaign(db: Db, { reset = false } = {}): Promise<string | null> {
  return db.transaction(async (tx) => {
    ...
    return insertBrinecross(tx, { name: DEMO_CAMPAIGN_NAME, includeCharacter: true });
  });
}

/** Creates a fresh Brinecross campaign. Without the pre-made character, the player builds their own. */
export async function createBrinecrossCampaign(db: Db, opts: { name: string; includeCharacter: boolean }): Promise<string> {
  return db.transaction((tx) => insertBrinecross(tx, opts));
}

async function insertBrinecross(tx: Db, { name, includeCharacter }: { name: string; includeCharacter: boolean }): Promise<string> {
  const [campaign] = await tx
    .insert(campaigns)
    .values({ name, worldBible: WORLD_BIBLE, narratorStyle: NARRATOR_STYLE, currencyName: 'crowns' })
  ...
  // These histories are Kael's; a custom character starts with no relationships.
  if (includeCharacter) await tx.insert(relationships).values([
    {
      campaignId,
  ...
  if (!includeCharacter) return campaignId;
